...
function performanceCheckNodePatternDCM4
...
function [FC, dlGC, gcI] = checkingPattern(pP,M,U,N,T,n,TR,options,idx)
    ...
    for k=1:N
        netFile = ['results/net-pat4-'  num2str(n) 'x' num2str(T) '-idx' num2str(idx) '-' num2str(k) '.mat'];
        netDLCM = [];
    ...
    %        figure; clims = [-1 1]; imagesc(Z,clims); title('PC - PLSPC');
            figure(plspcRf); hold on; [plspcROC{k,1}, plspcROC{k,2}, plspcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('PLS-PC');
        end
    ...
    %        figure; clims = [-1 1]; imagesc(Z,clims); title('PC - PCAPC');
            figure(pcpcRf); hold on; [pcpcROC{k,1}, pcpcROC{k,2}, pcpcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('PCA-PC');
        end
    ...
    %        figure; clims = [-1 1]; imagesc(Z,clims); title('PC - LassoPC');
            figure(lsopcRf); hold on; [lsopcROC{k,1}, lsopcROC{k,2}, lsopcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('Lasso-PC');
        end
        % show result of SVR-PC
        if isempty(svlpcROC{k,1})
            PC2 = calcSvPartialCorrelation(y2.',[], [], [], 'linear');
            figure(svlpcRf); hold on; [svlpcROC{k,1}, svlpcROC{k,2}, svlpcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('SVl-PC');
        end
        if isempty(svgpcROC{k,1})
            PC2 = calcSvPartialCorrelation(y2.',[], [], [], 'gaussian');
            figure(svgpcRf); hold on; [svgpcROC{k,1}, svgpcROC{k,2}, svgpcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('SVg-PC');
        end
        if isempty(svrpcROC{k,1})
            PC2 = calcSvPartialCorrelation(y2.',[], [], [], 'rbf');
            figure(svrpcRf); hold on; [svrpcROC{k,1}, svrpcROC{k,2}, svrpcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('SVr-PC');
        end
        % show result of GP-PC
        if isempty(gppcROC{k,1})
            PC2 = calcGpPartialCorrelation(y2.');
            figure(gppcRf); hold on; [gppcROC{k,1}, gppcROC{k,2}, gppcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('GP-PC');
        end
        % show result of Tree-PC
        if isempty(trpcROC{k,1})
            PC2 = calcTreePartialCorrelation(y2.');
            figure(trpcRf); hold on; [trpcROC{k,1}, trpcROC{k,2}, trpcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('Tree-PC');
        end
        % show result of RF-PC
        if isempty(rfpcROC{k,1})
            PC2 = calcRfPartialCorrelation(y2.');
            figure(rfpcRf); hold on; [rfpcROC{k,1}, rfpcROC{k,2}, rfpcAUC(k)] = plotROCcurve(PC2, pP.A, 100, 1, 0.2); hold off;
            title('RF-PC');
        end
        % show result of WCS
        ...
        if isempty(pcdlROC{k,1})
            ...
            figure(pcdlRf); hold on; [pcdlROC{k,1}, pcdlROC{k,2}, pcdlAUC(k)] = plotROCcurve(dlGC, pP.A, 100, 1, 0.2); hold off;
            title('PC-VARDNN-GC');
        end
        ...
        if isempty(pcdlwROC{k,1})
            ...
            figure(pcdlwRf); hold on; [pcdlwROC{k,1}, pcdlwROC{k,2}, pcdlwAUC(k)] = plotROCcurve(dlw, pP.A, 100, 1, 0.2); hold off;
            title('PC-VARDNN-DI');
        end
    end
