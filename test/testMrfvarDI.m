...
function testMrfvarDI
    ...
    %% test pattern 1 
    netMVAR = initMrfvarNetwork(si, exSignal, [], exControl, lags);
    % show multivaliate MRFVAR-DI
    figure; mDI = plotMrfvarDI(netMVAR, [], exControl, 0);
    figure; MIV = plotMrfvarMIV(si, exSignal, [], exControl, netMVAR, 0);
    % compare to mvGC
    ...
    netMVAR = initMrfvarNetwork(si, exSignal, [], exControl, lags);
    % show multivaliate MRFVAR-DI
    figure; mDI = plotMrfvarDI(netMVAR, [], exControl, 0, 1);
    figure; MIV = plotMrfvarMIV(si, exSignal, [], exControl, netMVAR, 0, 1);
    % compare to mvGC
    ...
    % init MRFVAR network
    netMVAR = initMrfvarNetwork(si, exSignal, nodeControl, exControl, lags);
    % show multivaliate MRFVAR-DI
    figure; mDI = plotMrfvarDI(netMVAR, nodeControl, exControl, 0, 1);
    figure; MIV = plotMrfvarMIV(si, exSignal, nodeControl, exControl, netMVAR, 0, 1);
    % compare to mvGC
