...
function simulateAlzheimerDLCM2
    ...
    [rccnDLs, meanRcCnDL, ~] = calculateConnectivity(cnSignals, roiNames, 'cn', 'dlcmrc', 0, 1, 1); % do recovery training
    [rcadDLs, meanRcAdDL, ~] = calculateConnectivity(adSignals, roiNames, 'ad', 'dlcmrc', 0, 1, 1); % do recovery training
...
%    [smcn6dDLWs, smcn6dSubDLWs, smcn6dSignals] = shiftAndExpandAmplitude(cnSignals, cnDLWs, cnSubDLWs, smcnSignals, smcnDLWs, smcnSubDLWs, 'smcn', 5);
    ...
    meanSmcn6bDLW = nanmean(smcn6bDLWs,3);
    % -- change simulated signals of high frequency (1:20) (wavelet) -> calc EC
    % -- 
    [smcn6fDLWs, smcn6fSubDLWs, smcn6fSignals] = shiftAndExpandAmplitude(cnSignals, cnDLWs, cnSubDLWs, smcnSignals, smcnDLWs, smcnSubDLWs, 'smcn', 7);
...
function [shiftDLWs, shiftSubDLWs, shiftSignals] = shiftAndExpandAmplitude(signals, DLWs, subDLWs, simSignals, simDLWs, simSubDLWs, group, type)
    ...
    sfName = ['results/adsim2/adsim2-shiftAmp' num2str(type) '-' group '-all.mat'];
    ...
    if type == 6, amps = [1,1.5,2,2.5,3,3.5,4,4.5,5]; nMax = length(amps); end
    if type == 7, amps = [1,1.5,2,2.5,3,3.5,4,4.5,5]; nMax = length(amps); end
...
%        hold off; title(['sbj' num2str(k) ' simulating signals']);
        ...
        outfName = ['results/adsim2/adsim2-shiftAmp' num2str(type) '-' group '-' num2str(k) '.mat'];
...
%            hold off; title(['sbj' num2str(k) ' shifted simulating signals']);
            ...
            if isnan(corrZi2(n))
                ...
                dlcmName = ['results/ad/ad-dlcm_ex-' group '-roi' num2str(nodeNum) '-net' num2str(k) '.mat'];
                f = load(dlcmName);
...
function checkRelationSubDLWandWeights(signals, subDLWs, smSubDLWs, group, type)
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation7-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            dlcmName = ['results/ad/ad-dlcm_ex-' group '-roi' num2str(nodeNum) '-net' num2str(k) '.mat'];
...
function [weDLWs, weSubDLWs, weSignals, weDLs] = checkRelationSubDLWandWeights2(signals, DLWs, subDLWs, smSignals, smDLWs, smSubDLWs, smDLs, group, type)
    ...
    sfName = ['results/adsim2/adsim2-checkRelation8' typename '-' group '-all.mat'];
    if exist(sfName, 'file')
        load(sfName);
        return;
    end
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation8' typename '-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            tmpfName = ['results/adsim2/adsim2-checkRelation8' typename '-' group '-' num2str(k) '_tmp.mat'];
...
function [wtDLWs, wtSubDLWs, wtSignals, wtDLs] = checkWaveletTransformEffect(signals, DLWs, subDLWs, group, type)
    ...
    sfName = ['results/adsim2/adsim2-checkWavelet' typename '-' group '-all.mat'];
    if exist(sfName, 'file')
        load(sfName);
        return;
    end
...
function [ampDLWs, ampSubDLWs, ampSignals, ampDLs] = checkRelationSubDLWandSignals3(signals, DLWs, subDLWs, smSignals, smDLWs, smSubDLWs, smDLs, group, type)
    ...
    sfName = ['results/adsim2/adsim2-checkRelation6' typename '-' group '-all.mat'];
    if exist(sfName, 'file')
        load(sfName);
        return;
    end
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation6' typename '-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            tmpfName = ['results/adsim2/adsim2-checkRelation6' typename '-' group '-' num2str(k) '_tmp.mat'];
            if exist(tmpfName, 'file')
                load(tmpfName);
...
function [ampDLWs, ampSubDLWs, ampSignals, ampDLs] = checkRelationSubDLWandSignals3b(signals, DLWs, subDLWs, smSignals, smDLWs, smSubDLWs, smDLs, group, orgGroup)
    ...
    sfName = ['results/adsim2/adsim2-checkRelation6b-' group '-all.mat'];
    if exist(sfName, 'file')
        load(sfName);
        return;
    end
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation6b-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            EC2s = cell(ampsLen,1);
            ...
            ampfName = ['results/adsim2/adsim2-checkRelation6-' orgGroup '-' num2str(k) '.mat'];
            af=load(ampfName);        

            dlcmName = ['results/ad/ad-dlcm_ex-' orgGroup '-roi' num2str(nodeNum) '-net' num2str(k) '.mat'];
            f = load(dlcmName);
            if isfield(f,'inSignal'), f.exSignal = f.inSignal; end % for compatibility
            if isfield(f,'inControl'), f.exControl = f.inControl; end % for compatibility
            exSignal = f.exSignal;
...
%    amps = [0, 0.01, 0.05, 0.1, 0.2, 0.5, 1, 1.2, 1.5, 2, 3, 5, 8];
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation4-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            tmpfName = ['results/adsim2/adsim2-checkRelation4-' group '-' num2str(k) '_tmp.mat'];
            if exist(tmpfName, 'file')
                load(tmpfName);
                if size(X,1) < R
                    X(R, ampsLen, nMax) = 0;
...
%}
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation5-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            Zi2 = zeros(R, ampsLen, nMax);
...
function checkRelationSubDLWandSignals(signals, DLWs, subDLWs, group, isRaw)
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            % if you want to use parallel processing, set NumProcessors more than 2
            ...
            tmpfName = ['results/adsim2/adsim2-checkRelation-' group '-' num2str(k) '_tmp.mat'];
            if exist(tmpfName, 'file')
                load(tmpfName);
                if size(X,1) < R
                    X(R,nMax,17) = 0;
...
%{
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation2-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            % if you want to use parallel processing, set NumProcessors more than 2
...
%}
    ...
    for k=1:sbjMax
        ...
        outfName = ['results/adsim2/adsim2-checkRelation3-' group '-' num2str(k) '.mat'];
        if exist(outfName, 'file')
            load(outfName);
        else
            % if you want to use parallel processing, set NumProcessors more than 2
            ...
            tmpfName = ['results/adsim2/adsim2-checkRelation3-' group '-' num2str(k) '_tmp.mat'];
            if exist(tmpfName, 'file')
                load(tmpfName);
                if size(X,1) < R
                    X(R,nMax,ampsLen) = 0;
