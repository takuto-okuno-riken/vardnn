%%
% Plotting mPLSVAR (multivaliate PLS Vector Auto-Regression) Granger Causality
...
function [gcI, h, P, F, cvFd, AIC, BIC, nodeAIC, nodeBIC] = plotMplsvarGCI(X, exSignal, nodeControl, exControl, net, range, alpha, isFullNode)
    ...
    [gcI, h, P, F, cvFd, AIC, BIC, nodeAIC, nodeBIC] = calcMplsvarGCI(X, exSignal, nodeControl, exControl, net, alpha, isFullNode);
    if range <= 0
    ...
    imagesc(gcI2,clims);
    daspect([1 1 1]);
    title('mPLSVAR Granger Causality Index');
    xlabel('Source Nodes');
    ylabel('Target Nodes');
    colorbar;
end
