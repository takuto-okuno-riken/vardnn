...
function [gcI, h, P, F, cvFd, AIC, BIC, nodeAIC, nodeBIC] = plotMultivariateGCI2(X, exSignal, nodeControl, exControl, lag, range, alpha, isFullNode)
    if nargin < 8
        isFullNode = 0;
    end
    if nargin < 7
    ...
    [gcI, h, P, F, cvFd, AIC, BIC, nodeAIC, nodeBIC] = calcMultivariateGCI2(X, exSignal, nodeControl, exControl, lag, alpha, isFullNode);
    if range <= 0
