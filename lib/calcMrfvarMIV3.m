%%
% Calculate multivariate Random Forest VAR Mean Impact Value of 3D matrix (Full Input)
% returns multivariate Random Forest VAR Mean Impact Value 3D matrix (MIV) and Mean Absolute Impact Value 3D matrix (MAIV)
% input:
%  X            multivariate time series matrix (node x time series)
%  exSignal     multivariate time series matrix (exogenous input x time series) (optional)
%  nodeControl  node control matrix (node x node) (optional)
%  exControl    exogenous input control matrix for each node (node x exogenous input) (optional)
%  net          trained multivariate RF VAR network
%  isFullNode   return both node & exogenous causality matrix (default:0)

function [MIV, MAIV] = calcMrfvarMIV3(X, exSignal, nodeControl, exControl, net, isFullNode)
    ...
    for i=1:nodeNum
        ...
        for j=1:nodeMax
            for p=1:lags
                ...
                % predict 
                N1 = predict(net.mdl{i}, Xj1);
                N2 = predict(net.mdl{i}, Xj2);
