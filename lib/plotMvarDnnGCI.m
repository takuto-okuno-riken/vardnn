%%
% Plotting multivariate VAR DNN Granger causality Index matrix
...
function [gcI, h, P, F, cvFd, AIC, BIC, nodeAIC, nodeBIC] = plotMvarDnnGCI(X, exSignal, nodeControl, exControl, net, range, alpha, isFullNode)
    if nargin < 8, isFullNode = 0; end
    if nargin < 7, alpha = 0.05; end
    if nargin < 6, range = 10; end

    [gcI, h, P, F, cvFd, AIC, BIC, nodeAIC, nodeBIC] = calcMvarDnnGCI(X, exSignal, nodeControl, exControl, net, alpha, isFullNode);
    if range <= 0
    ...
    imagesc(gcI2,clims);
    daspect([1 1 1]);
    title('multivariate VAR DNN Granger Causality');
    xlabel('Source Nodes');
    ylabel('Target Nodes');
    colorbar;
end
