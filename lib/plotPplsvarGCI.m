%%
% Plotting pPLSVAR (pairwise PLS Vector Auto-Regression) Granger Causality
...
function [gcI, h, P, F, cvFd, AIC, BIC] = plotPplsvarGCI(X, exSignal, nodeControl, exControl, net, range, alpha, isFullNode)
    ...
    if nargin < 3, exSignal = []; end

    [gcI, h, P, F, cvFd, AIC, BIC] = calcPplsvarGCI(X, exSignal, nodeControl, exControl, net, alpha, isFullNode);
    if range <= 0
    ...
    imagesc(gcI2,clims);
    daspect([1 1 1]);
    title('pPLSVAR Granger Causality Index');
    xlabel('Source Nodes');
    ylabel('Target Nodes');
    colorbar;
end
