%%
% Plotting nVARNN Mean Impact Value (MIV) matrix
% returns nVARNN MIV matrix (MIV)
% input:
...
function MIV = plotNvarnnMIV(X, exSignal, nodeControl, exControl, net, range, isFullNode)
    if nargin < 7, isFullNode = 0; end
    if nargin < 6, range = 1; end

    MIV = calcNvarnnMIV(X, exSignal, nodeControl, exControl, net, isFullNode);
    if range <= 0
    ...
    imagesc(MIV2,clims);
    daspect([1 1 1]);
    title('nVARNN Mean Impact Value');
    xlabel('Source Nodes');
    ylabel('Target Nodes');
    colorbar;
end
