%%
% Plot Convergent Cross Mapping Multivariate Granger Causality
...
function [CCM, P] = plotConvCrossMapMGC(X, exSignal, nodeControl, exControl, E, tau, range, alpha, isFullNode)
    ...
    [CCM, P] = calcConvCrossMapMGC_(X, exSignal, nodeControl, exControl, E, tau, alpha, isFullNode);
    if range <= 0
    ...
    imagesc(gcI2,clims);
    daspect([1 1 1]);
    title(['Convergent Cross Mapping multivariate GC Index (E=' num2str(E) ', tau=' num2str(tau) ')']);
    xlabel('Source Nodes');
    ylabel('Target Nodes');
    colorbar;
end
